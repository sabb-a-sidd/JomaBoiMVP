...
class _PaymentForm extends State<PaymentForm>{
  ...
  void handleSaveTransaction(context) async {
    ...
    try {
      ...
      // Notify about the update before popping
      globalEvent.emit("payment_update");
      
      Navigator.of(context).pop();
      
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${_id == null ? 'Added' : 'Updated'} successfully')),
      );
    } catch (e) {
      debugPrint('Error saving payment: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error saving payment. Please try again.')),
      );
    }
  }
